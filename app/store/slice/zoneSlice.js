...
export const getAllNewZones = createAsyncThunk(
  ...
  async (_, thunkAPI) => {
    try {
      const response = await FetchApi({
        endpoint: "/user/zone/getAllNewZones",
        method: "GET",
      });
      ...
      return thunkAPI.rejectWithValue(
        err.message || "Failed to fetch new zones",
      );
    }
  },
);
...
const zonesSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getAllNewZones.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(getAllNewZones.fulfilled, (state, action) => {
        state.loading = false;
        state.newZones = action.payload?.zones || [];
      })
      .addCase(getAllNewZones.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload;
      });
  },
