...
const CommonHeroSection = ({
  ...
  backgroundImage,
  breadcrumbs = [],
  height = "min-h-[250px] sm:min-h-[300px] md:h-[350px] lg:h-[400px]",
  overlay = "bg-gradient-to-t from-black/50 via-black/20 to-black/20",
}) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative text-center text-white px-4 sm:px-6 py-10 max-w-4xl z-10 w-full">
        <h3
          className={`text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold drop-shadow-2xl capitalize overflow-hidden leading-tight transition-all duration-1000 ${textVisible ? "translate-y-0 opacity-100" : "translate-y-12 opacity-0"
        ...
        {subtitle && (
          <p
            className={`mt-4 sm:mt-6 text-sm sm:text-base md:text-lg text-white/95 drop-shadow-2xl transition-all duration-1000 delay-500 max-w-2xl mx-auto ${textVisible ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0"
        ...
        {breadcrumbs?.length > 0 && (
          <div
            className={`mt-4 transition-all duration-1000 delay-[800ms] flex flex-wrap justify-center items-center gap-2 sm:gap-3 ${textVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
              }`}
          >
